  ...
  Widget build(BuildContext context) {
    ...
    return permissionVM.permissionStatus == PermissionStatus.loaded
        ? Scaffold(
          ...
          body:
              permissionVM.permissionListItems.isNotEmpty
                  ? ListView.builder(
                    physics: const BouncingScrollPhysics(),
                    padding: context.paddingLow,
                    itemCount: permissionVM.permissionListItems.length,
                    itemBuilder:
                        (context, int index) => PermissionListTile(
                          type: permissionVM.permissionListItems[index].type,
                          confirmText:
                              permissionVM
                          ...
                          typeText:
                              ...
                              strCons.unSpecified,
                          approvalStatus:
                              permissionVM.permissionListItems[index].confirm ??
                              0,
                          address:
                              permissionVM.permissionListItems[index].address ??
                              strCons.unSpecified,
                          reasonText:
                              permissionVM.permissionListItems[index].reason ??
                              strCons.unSpecified,
                        ),
                  )
                  : Center(
